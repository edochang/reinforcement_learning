...
class BanditEnvironment(gym.Env):
    ...
    def _sample_reward(self, action: int) -> float:
        ...
        returns:
            float: the reward of the selected arm
        """
        ### ✅TODO ###
        ...
        return np.random.normal(loc=self.Q_star[action], scale=1.0, size=None)
    ...
    def _is_ideal_action(self, action: int) -> bool:
        ...
        max_Q_star = np.max(self.Q_star)
        return self.Q_star[action] == max_Q_star
    ...
    def _walk_all_arms(self):
        ...
        self.Q_star = np.random.normal(loc=self.Q_star, scale=0.01, size=self.action_space.n)
